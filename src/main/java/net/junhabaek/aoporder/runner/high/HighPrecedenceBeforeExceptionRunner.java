...
@Slf4j
@Order(value=2)
public class HighPrecedenceBeforeExceptionRunner implements ApplicationRunner {
    private final StudentService studentService;

    @Override
    public void run(ApplicationArguments args) throws Exception {
        String exceptionStudentName = "high-before";

        runServiceWithException(exceptionStudentName);
        checkStudentNotExist(exceptionStudentName);
        log.info("high-before completed with expected result");
    }

    private void runServiceWithException(String name){
        try{
            studentService.createStudentWithHighPrecedenceBeforeException(name);
        }
        catch(Throwable t){
    ...
    private void checkStudentNotExist(String name){
